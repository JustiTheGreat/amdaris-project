...
namespace AmdarisProject.Application.Test
{
    public class AddPlayerToTeamHandlerTest
    {
        ...
        public async Task Test_AddPlayerToTeamHandler_Success()
        {
            ...
            _competitorRepositoryMock.SetupSequence(o => o.GetById(It.IsAny<ulong>()))
            ...
            _competitorRepositoryMock.Verify(o => o.Update(It.Is<Team>(t => t.Players.Any(p => p.Id == player.Id))), Times.Once);
            Assert.Equal(team.Id, response.Id);
            Assert.Equal(player.Id, response.Players.ElementAt(response.Players.Count - 1));
        }
        ...
        public async Task Test_AddPlayerToTeamHandler_TeamNotFound_Throws_APNotFoundException()
        {
            ...
            AddPlayerToTeam command = new(It.IsAny<ulong>(), It.IsAny<ulong>());
            AddPlayerToTeamHandler handler = new(_unitOfWorkMock.Object, _mapperMock.Object);
