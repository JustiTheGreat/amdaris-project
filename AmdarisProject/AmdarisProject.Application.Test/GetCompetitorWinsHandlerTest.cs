...
namespace AmdarisProject.Application.Test
{
    public class GetCompetitorWinsHandlerTest
    {
        ...
        public async Task Test_GetCompetitorWinsHandler_Success()
        {
            ...
            _matchRepositoryMock.Setup(o => o.GetAllByCompetitorAndGameType(It.IsAny<ulong>(), It.IsAny<GameType>()))
                .Returns(Task.FromResult((IEnumerable<Match>)matches));
            var v = _matchRepositoryMock.SetupSequence(o => o.GetById(It.IsAny<ulong>()));
            for (int i = 0; i < numberOfMatches; i++)
                v.Returns(Task.FromResult((Match?)matches[i])).
                    Returns(Task.FromResult((Match?)matches[i]))
                    .Returns(Task.FromResult((Match?)matches[i]));

            var v2 = _competitorRepositoryMock.SetupSequence(o => o.GetById(It.IsAny<ulong>()));
            for (int i = 0; i < numberOfMatches; i++)
                v2.Returns(Task.FromResult((Competitor?)matches[i].CompetitorOne))
                    .Returns(Task.FromResult((Competitor?)matches[i].CompetitorTwo));

            var v3 = _pointRepositoryMock.SetupSequence(o => o.GetByPlayerAndMatch(It.IsAny<ulong>(), It.IsAny<ulong>()));
            for (int i = 0; i < numberOfMatches; i++)
            ...
            _matchRepositoryMock.Verify(o => o.GetAllByCompetitorAndGameType(It.IsAny<ulong>(), It.IsAny<GameType>()), Times.Once);
            _matchRepositoryMock.Verify(o => o.GetById(It.IsAny<ulong>()), Times.Exactly(3 * numberOfMatches));
            _competitorRepositoryMock.Verify(o => o.GetById(It.IsAny<ulong>()), Times.Exactly(2 * numberOfMatches));
            _pointRepositoryMock.Verify(o => o.GetByPlayerAndMatch(It.IsAny<ulong>(), It.IsAny<ulong>()), Times.Exactly(2 * numberOfMatches));
        }
        ...
        public async Task Test_GetCompetitorWinsHandler_MatchNotFound_Throws_AggregateException()
        {
            ...
            GetCompetitorWins command = new(It.IsAny<ulong>(), It.IsAny<GameType>());
            GetCompetitorWinsHandler handler = new(_unitOfWorkMock.Object);
