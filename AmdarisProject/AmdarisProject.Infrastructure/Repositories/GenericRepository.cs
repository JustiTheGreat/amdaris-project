...
namespace AmdarisProject.Infrastructure.Repositories
{
        ...
        : IGenericRepository<T> where T : Model
    {
        ...
        public async Task<IEnumerable<T>> GetByIds(IEnumerable<ulong> ids)
            ...
            ?? throw new APNotFoundException(nameof(GenericRepository<T>), nameof(GetByIds), [Tuple.Create(nameof(id), id)])).ToList();
        ...
        public async Task Delete(ulong id)
            => _dbContext.Set<T>().Remove(await GetById(id)
                ?? throw new APNotFoundException(nameof(GenericRepository<T>), nameof(Delete), [Tuple.Create(nameof(id), id)]));

        public async Task<T> Update(T item)
        {
            if (item is null)
                throw new APArgumentException(nameof(GenericRepository<T>), nameof(Update), nameof(item));

            T stored = await GetById(item.Id)
                ?? throw new APNotFoundException(nameof(GenericRepository<T>), nameof(Update), [Tuple.Create(nameof(item.Id), item.Id)]);
